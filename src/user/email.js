...
UserEmail.remove = async function (uid, sessionId) {
  ...
  await Promise.all([user.setUserFields(uid, {
    email: '',
    'email:confirmed': 0
  }), db.sortedSetRemove('email:uid', email.toLowerCase()), db.sortedSetRemove('email:sorted', `${email.toLowerCase()}:${uid}`), user.email.expireValidation(uid), sessionId ? user.auth.revokeAllSessions(uid, sessionId) : Promise.resolve(), events.log({
    targetUid: uid,
    type: 'email-change',
    email,
    newEmail: ''
