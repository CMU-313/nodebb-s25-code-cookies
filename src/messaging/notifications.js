...
module.exports = function (Messaging) {
  ...
  async function sendNotification(fromUid, roomId, messageObj) {
    ...
    await batch.processSortedSet(`chat:room:${roomId}:uids:online`, async uids => {
      uids = uids.filter(uid => parseInt(settings && settings[uid] || roomDefault, 10) === ALLMESSAGES && fromUid !== parseInt(uid, 10) && !realtimeUids.includes(parseInt(uid, 10)));
    ...
    if (uidsToNotify.length) {
      ...
      const notifData = {
        ...
        // as roomId is the differentiator, no distinction between direct vs. group req'd.
        from: fromUid,
        roomId,
        roomName,
